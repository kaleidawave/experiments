use general_parser::{
	Adjacency, BinaryOperator, UnaryOperator, Configuration, Expression,
};
...
fn main() {
	...
	let configuration = Configuration {
		prefix_unary_operators: vec![
			UnaryOperator { representation: "-", precedence: 2 }
		],
	...
	for source in sources {
		...
		let expression: Expression<&str> = Expression::from_string(source, &configuration, &allocator);
		eprintln!("{expression:#?}");
	}
}
